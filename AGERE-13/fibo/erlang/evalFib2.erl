-module(evalFib2).
-export([main/0, client/3, fibonaccer/0, manager/1, subActor/0]).

-define(NAG, 100).
-define(NASK, 30).
-define(MAXITH, 38).
-define(FILENAME, "data-erlang2.csv").

spawn_many(F,M) -> spawn_many(1,F,M).

spawn_many(?NAG,F,M) -> C = spawn(evalFib2, client, [F,?NASK,M]),
                 C ! "ask";


spawn_many(A,F,M) -> C = spawn(evalFib2, client, [F,?NASK,M]),
                 C ! "ask",
                 spawn_many(A+1,F,M).
...
subActor() ->
 ...
 fibonaccer() ->
   receive
	{N, Pid, Start} -> 	Sact = spawn(evalFib2, subActor, []),
			Sact ! {N, Pid, Start},
		        fibonaccer()
   end.
...
main() ->
   ...
   F = spawn(evalFib2, fibonaccer, []),
   M = spawn(evalFib2, manager, [1]),
   spawn_many(F,M).
